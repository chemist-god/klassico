...
import { CheckCircle2, Printer, ArrowLeft, Copy, ShoppingBag } from "lucide-react";
...
export function OrderReceipt({ order, isNew = false }: OrderReceiptProps) {
    ...
    return (
        <div className="w-full max-w-3xl mx-auto animate-in fade-in zoom-in duration-500">
            ...
            {/* Receipt Card */}
            <Card className="border-border/50 shadow-2xl overflow-hidden bg-card/80 backdrop-blur-xl print:shadow-none print:border-none">

                {/* Receipt Header (Visible especially on Print) */}
                <div className="bg-primary/5 p-6 border-b border-border/50 flex justify-between items-start">
                    <div>
                        <Link href="/" className="flex items-center gap-2 mb-2">
                            <div className="h-8 w-8 rounded-lg bg-primary flex items-center justify-center">
                                <ShoppingBag className="h-5 w-5 text-primary-foreground" />
                            </div>
                            <span className="font-bold text-xl tracking-tight">Kubera</span>
                        </Link>
                        <div className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Official Receipt</div>
                    </div>
                ...
                <CardContent className="p-8 space-y-8">
                    {/* Order Info Bar */}
                    <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center p-4 rounded-xl bg-muted/30 border border-border/40">
                        <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Button variant="ghost" size="icon" className="h-6 w-6 opacity-50 hover:opacity-100" onClick={copyOrderId}>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-12 text-xs uppercase tracking-wider text-muted-foreground px-2 pb-2 border-b border-border/30">
                                <div className="col-span-8 md:col-span-6">Item</div>
                                <div className="col-span-2 text-center md:hidden">Qty</div>
                            ...
                            {order.items.map((item: OrderItem) => (
                                <div key={item.id} className="grid grid-cols-12 items-center py-3 px-2 border-b border-border/30 last:border-0 hover:bg-muted/30 transition-colors rounded-lg">
                                    <div className="col-span-8 md:col-span-6">
                                        <span className="font-medium text-foreground block">{item.product.name}</span>
                                        <span className="text-xs text-muted-foreground md:hidden">${item.price.toLocaleString()} each</span>
                                    </div>
                                    <div className="col-span-2 text-center text-xs md:text-sm md:hidden">x{item.quantity}</div>
